...
class FAM(nn.Module):
    def __init__(self, size, kernel_size=7, DC=True):
        super(FAM, self).__init__()
        self.DC = DC
        self.size = size
        self.DCT = nn.Parameter(torch.tensor(self.DCT_mat(self.size)).float(), requires_grad=False)
        self.DCT_T = nn.Parameter(torch.transpose(torch.tensor(self.DCT_mat(self.size)).float(), 0, 1),
                                  requires_grad=False)
    ...
    def forward(self, x):
        x_freq = self.DCT @ x @ self.DCT_T
        if not self.DC:  # 是否去除直流分量
            x_freq[..., 0, 0] = 0

        avgout = torch.mean(x_freq, dim=1, keepdim=True)
        maxout, _ = torch.max(x_freq, dim=1, keepdim=True)
        mask = torch.cat([avgout, maxout], dim=1)
        mask = self.conv(mask)
        att = self.sigmoid(mask)

        x_freq = x_freq * att
        x = self.DCT_T @ x_freq @ self.DCT
        return x
...
class Xception(nn.Module):
    ...
    def __init__(self, num_classes=1):
        ...
        self.fam0 = FAM(size=299, DC=False)
        ...
        self.fam2 = FAM(size=74)
        self.block2 = Block(128, 256, 2, 2, start_with_relu=True, grow_first=True)
        self.fam3 = FAM(size=37)
        self.block3 = Block(256, 728, 2, 2, start_with_relu=True, grow_first=True)

        self.fam4 = FAM(size=19, kernel_size=3)
        ...
        self.fam12 = FAM(size=19, kernel_size=3)
