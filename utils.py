...
class FFDataset(data.Dataset):
    def __init__(self, dataset_root, frame_num=300, size=299, augment=None):
    ...
    def get_mask(self, path):
        # example: path=/data/manipulated_images/Deepfakes/c40/train/005_010/0001.png
        info = path.split("/")
        if info[-5] == "youtube":
    ...
    def __getitem__(self, index):
        ...
        if self.albus:
            outs = self.albus(image=np.array(img), mask=np.array(mask))
            img = Image.fromarray(outs["image"])
            mask = Image.fromarray(outs["mask"])
...
def get_dataset(name='train', size=299, root='../data', frame_num=300, augment=None, compress="c40", fake_list=None):
    ...
    for i in range(total_len):
        fake = os.path.join(root, fake_list[i], compress, name)
...
def evaluate(model, data_path, mode='val', compress="c40", fake_list=None):
    root= data_path
    origin_root = root
    root = os.path.join(data_path, "youtube", compress, mode)
    dataset_real = FFDataset(dataset_root=root, size=299, frame_num=50, augment=None)
    dataset_fake, _ = get_dataset(name=mode, root=origin_root, size=299, frame_num=50, augment=None, compress=compress, fake_list=fake_list)
    ...
    while thresh < 1:
        ...
        xx[xx >= thresh] = 1
        xx[xx < thresh] = 0
        acc_tmp = accuracy_score(y_true, xx)
        acc = max(acc, acc_tmp)
        thresh += 0.01
